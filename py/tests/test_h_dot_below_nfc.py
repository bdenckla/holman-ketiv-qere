...
"""Enforcement test (issue #187 convention): hand-authored source must
standardize Latin transliteration of Hebrew het ("h with dot below") on
NFC, i.e. the precomposed U+1E25 / U+1E24 forms, never the decomposed
"h"/"H" + COMBINING DOT BELOW (U+0323) sequence. Comments must not use
either Unicode form at all -- plain ASCII "x"/"X" is used instead, since
comments don't flow to output.
...
rafeh) according to Unicode canonical combining class, which conflicts with
this repo's own documented, deliberate non-Unicode-standard Hebrew mark
order (see mb_cmn/uni_denorm.py). So this test checks only the specific
h-with-dot-below sequence, which has no such ambiguity: composing "h"/"H" +
U+0323 to U+1E25/U+1E24 is a simple, unambiguous Latin-script composition.

Scope also excludes:
  - out/ (generated report output, not hand-authored)
  - gh-pages/ (generated GitHub Pages output: HTML, CSS/JS, and images
    emitted by the repo's main_*.py render pipeline)
...
# Generated-output directory prefixes: out/ (report JSON) and gh-pages/
# (GitHub Pages HTML/CSS/JS/images), both emitted by the repo's render
# pipeline. Regenerating them in non-NFC form is a separate, future concern.
...
class TestHDotBelowNfc(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # accidentally excludes everything). This repo has ~80 in-scope
        # text files; 40 is a comfortable floor below that.
